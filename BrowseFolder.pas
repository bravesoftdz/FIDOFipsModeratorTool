...
{ TItemIDList -- List if item IDs (combined with 0-terminator) }
  ...
  TFNBFFCallBack = function(Wnd: HWND; uMsg: UINT; lParam, lpData: LPARAM):
Integer stdcall;
...
{ Browsing for directory }
  ...
  BIF_RETURNONLYFSDIRS   = $0001; { For finding a folder to start document
searching }
...
function SHGetSpecialFolderLocation(hwndOwner: HWND; nFolder: Integer;
  ...
  function Browse(Handle:HWnd;Directory: string): string;

implementation

function ExplorerHook(Wnd: HWnd; Msg: UINT; LParam: LPARAM; Data: LPARAM):
Integer; stdcall;
begin
  Result := 0;
...
begin
  ...
  begin
    ...
    pidlRoot := nil;
{Must take from lang!}
	 lpszTitle := 'Выберите папку для экспорта';
{Must take from lang!}
	 GetMem(p,255);
	 if directory <> '' then
	 begin
